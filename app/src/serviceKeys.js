// store
export const userStoreKey = Symbol();
export const configurationStoreKey = Symbol();
export const musicPlayerStoreKey = Symbol();
export const notificationStoreKey = Symbol();

// i18n
export const i18nKey = Symbol();

// services
export const authServiceKey = Symbol();
export const userServiceKey = Symbol();
export const musicServiceKey = Symbol();
export const recommendationServiceKey = Symbol();
export const playlistServiceKey = Symbol();